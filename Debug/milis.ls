 ...
 170  0031 81            	ret
 199                     ; 55 void init_milis(void){
 200                     	switch	.text
 201  0032               _init_milis:
 205                     ; 56 TIM4_TimeBaseInit(PRESCALER,PERIOD); // (16MHz / 128) / 125 = 1000Hz
 207  0032 ae077c        	ldw	x,#1916
 208  0035 cd0000        	call	_TIM4_TimeBaseInit
 210                     ; 57 TIM4_ClearFlag(TIM4_FLAG_UPDATE);
 212  0038 a601          	ld	a,#1
 213  003a cd0000        	call	_TIM4_ClearFlag
 215                     ; 58 TIM4_ITConfig(TIM4_IT_UPDATE,ENABLE);
 217  003d ae0101        	ldw	x,#257
 218  0040 cd0000        	call	_TIM4_ITConfig
 220                     ; 59 ITC_SetSoftwarePriority(ITC_IRQ_TIM4_OVF, ITC_PRIORITYLEVEL_1); // nízká priorita pøerušení
 222  0043 ae1701        	ldw	x,#5889
 223  0046 cd0000        	call	_ITC_SetSoftwarePriority
 225                     ; 60 TIM4_Cmd(ENABLE);
 227  0049 a601          	ld	a,#1
 228  004b cd0000        	call	_TIM4_Cmd
 230                     ; 61 enableInterrupts();
 233  004e 9a            rim
 235                     ; 62 }
 239  004f 81            	ret
 265                     ; 65 INTERRUPT TIM4_UPD_OVF_IRQHandler(){
 267                     	switch	.text
 ...
 280  0061 89            	pushw	x
 283                     ; 66 TIM4_ClearFlag(TIM4_FLAG_UPDATE);
